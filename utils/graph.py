...
def plot_graph(g: Graph, pos: dict | None = None):
    nx.draw(g, with_labels=True, pos=pos, node_color='lightblue', edge_color='gray', node_size=500, font_size=10)
    plt.show()
...
def plot_attention(g: Graph, attention: Float[Tensor, "n_heads n_vertices n_vertices"]):
    ...
    planar, cert = nx.check_planarity(g)
    pos = nx.combinatorial_embedding_to_pos(cert) if planar else nx.spring_layout(g)
    ...
    node_trace.text = [str(i) for i in range(len(g.nodes))]
